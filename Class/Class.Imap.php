...
class Imap
{
    ...
    {
        $connection = imap_open($hostname, $username, $password) or die(json_encode(array('status' => false, 'message' => 'Cannot connect to Mail: ' . imap_last_error())));
        $this->imapStream = $connection;
        return true;
    }

    public function getMessages($sort = 'asc', $filter)
    {
        if(!$filter || $filter == 'ALL') $query_filter = 'ALL'; else $query_filter = $filter;
        $stream = $this->imapStream;
        $this->emails = imap_search($stream, $query_filter);
        ...
        if ($this->emails) {
            ...
            foreach ($this->emails as $email_number) {
                ...
                $messages[] = $this->loadMessage($uid, $email_number);
                if ($i == $this->limit) {
                    break;
                }
                $i++;
    ...
    private function loadMessage($uid, $email_number)
    {
        ...
        $array['message'] = html_entity_decode(htmlentities($this->htmlMessage));
        $array['message'] = html_entity_decode(htmlentities($this->replaceInlineImagesSrcWithRealPath($array['message'])));
